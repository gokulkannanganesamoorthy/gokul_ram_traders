...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-fit px-4 md:px-0">
      ...
      {/* Desktop Pill Nav */}
      <nav aria-label="Main Navigation" className="hidden md:flex glass rounded-full p-1.5 items-center gap-1 shadow-2xl">
            ...
            {NAV_ITEMS.map(({ id, label }) => (
              <a
                ...
                className={`px-6 py-4 rounded-[20px] text-[10px] uppercase tracking-[0.2em] font-bold transition-all ${
                  active === id ? 'bg-brand-black text-brand-white' : 'text-brand-gray-400 hover:text-brand-black hover:bg-brand-gray-100'
                }`}
              >
                {label}
              </a>
